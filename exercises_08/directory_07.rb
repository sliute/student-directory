...
def input_students
  ...
  students = []
  cohort_months = ['january', 'february', 'march', 'april', 'june', 'july', 'august', 'september', 'october', 'november', 'december']
  # get the first name
  name = gets.chomp
  # provide a default value for name if the input is empty
  name = "Default Name" if name == ''
  cohort = gets.chomp
  cohort = "November" if cohort == '' || !cohort_months.include?(cohort.downcase)
